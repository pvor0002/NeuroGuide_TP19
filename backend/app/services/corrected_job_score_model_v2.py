...
semi-supervised targets (impairment vs synthetic job environment). Final score (unless ``ML_BLEND_WEIGHT`` is 0): ``ml_w * ML + (1-ml_w) * rule_tilt`` where ``ml_w`` is
0.30–0.60 from ML probability uncertainty (near 0.5 ⇒ favor rules), then nonlinear spread; display clamp
[0, 100] for debugging. When ``ML_BLEND_WEIGHT = 0`` and ``RULE_BLEND_WEIGHT = 1``, only rule tilt is used.
...
def _normalize_soft_demand_level(value: object) -> Optional[str]:
    ...
    if not s:
        return None
    # Exact tokens first (model sometimes echoes schema labels)
...
class JobScoreModelV2:
    ...
    # ── Hybrid blend (aggressive rebalance) ──────────────────────────────────────
    # Keep rule layer primary while retaining ML penalty for hard domain mismatches.
    ML_BLEND_WEIGHT = 0.20
    RULE_BLEND_WEIGHT = 0.80
    ...
    # Use raw rule score for blending (no compression toward 50).
    RULE_BLEND_COMPRESSION = 1.0
    # Primary/secondary/tertiary rule blend emphasis.
    SKILLS_WEIGHT = 0.50
    WORK_STYLE_WEIGHT = 0.25
    SOFT_SKILLS_WEIGHT = 0.25
    # Hard cap when technical/domain match is too low.
    DOMAIN_MISMATCH_SKILL_THRESHOLD = 30.0
    DOMAIN_MISMATCH_FINAL_CAP = 25.0
    ...
    def effective_ml_rule_blend_weights(ml_prob: Optional[float]) -> Tuple[float, float]:
        """
        Deterministic blend weights for the active configuration.
        """
        if JobScoreModelV2.ML_BLEND_WEIGHT <= 0.0 and JobScoreModelV2.RULE_BLEND_WEIGHT >= 1.0:
            return 0.0, 1.0
        total = float(JobScoreModelV2.ML_BLEND_WEIGHT + JobScoreModelV2.RULE_BLEND_WEIGHT)
        if total <= 0:
            return JobScoreModelV2.ML_BLEND_WEIGHT_BASE, JobScoreModelV2.RULE_BLEND_WEIGHT_BASE
        return JobScoreModelV2.ML_BLEND_WEIGHT / total, JobScoreModelV2.RULE_BLEND_WEIGHT / total
    ...
    @staticmethod
    def _factor_adjustment_to_pct(adjustment: Optional[float], max_points: Optional[float]) -> Optional[float]:
        mp = float(max_points or 0.0)
        if mp <= 0:
            return None
        adj = float(adjustment or 0.0)
        pct = ((adj + mp) / (2.0 * mp)) * 100.0
    ...
    def score_job_match(
        ...
        accumulated_rule_score = max(0.0, min(100.0, score))
        skills_score_pct = self._skills_alignment_pct(user_skills, job_skills)
        work_pref_pct = self._factor_adjustment_to_pct(
            factors.get("work_preferences", {}).get("adjustment"),
            factors.get("work_preferences", {}).get("max_points"),
        )
        energy_pct = self._factor_adjustment_to_pct(
            factors.get("energy_patterns", {}).get("adjustment"),
            factors.get("energy_patterns", {}).get("max_points"),
        )
        if work_pref_pct is None and energy_pct is None:
            work_style_score_pct = 50.0
        elif work_pref_pct is None:
            work_style_score_pct = float(energy_pct)
        elif energy_pct is None:
            work_style_score_pct = float(work_pref_pct)
        else:
            work_style_score_pct = (float(work_pref_pct) * 15.0 + float(energy_pct) * 10.0) / 25.0

        soft_skills_debug = self._build_soft_skills_debug(
            use_gemini_fit=use_gemini_fit,
        ...
        if soft_score_raw is not None and isinstance(soft_score_raw, (int, float)):
            ...
            soft_pct_fallback = self._factor_adjustment_to_pct(
                factors.get("soft_skills", {}).get("adjustment"),
                factors.get("soft_skills", {}).get("max_points"),
            )
            soft_skills_score_pct = float(soft_pct_fallback if soft_pct_fallback is not None else 50.0)

        weighted_rule_score = (
            self.SKILLS_WEIGHT * skills_score_pct
            + self.WORK_STYLE_WEIGHT * work_style_score_pct
            + self.SOFT_SKILLS_WEIGHT * soft_skills_score_pct
        )
        rule_score = max(0.0, min(100.0, weighted_rule_score))
        logger.info(
            "[JobScoreModelV2 DEBUG] occupation_id=%s rule_score=%.4f (weighted: skills=%.2f work_style=%.2f soft=%.2f)",
            occupation.get("occupation_id"),
            rule_score,
            skills_score_pct,
            work_style_score_pct,
            soft_skills_score_pct,
        )
        ...
        ml_layer: Dict = {
            ...
            "rule_score_raw_accumulated_0_100": round(accumulated_rule_score, 2),
            "skills_score_0_100": round(skills_score_pct, 2),
            "work_style_score_0_100": round(work_style_score_pct, 2),
            "soft_skills_score_0_100": round(soft_skills_score_pct, 2),
        ...
        if ml_prob is not None:
            ...
            if ml_w_eff <= 0.0:
                ml_layer["blend"] = (
                    "rules only — ML_BLEND_WEIGHT=0 (nuclear); compressed rule tilt only "
                    f"(ML prob still logged: {blend_ml_desc})"
                )
            else:
                ml_layer["blend"] = (
                    f"{ml_w_eff:.0%} ML ({blend_ml_desc}) + {rule_w_eff:.0%} compressed rule tilt "
                    "(constant configured blend)"
                )
            logger.info(
        ...
        if skills_score_pct < self.DOMAIN_MISMATCH_SKILL_THRESHOLD:
            final_score = min(final_score, self.DOMAIN_MISMATCH_FINAL_CAP)
            ml_layer["domain_mismatch_cap_applied"] = True
            ml_layer["domain_mismatch_cap_reason"] = (
                f"skills_score_0_100={skills_score_pct:.2f} < {self.DOMAIN_MISMATCH_SKILL_THRESHOLD:.0f}; "
                f"final capped to <= {self.DOMAIN_MISMATCH_FINAL_CAP:.0f}"
            )
        ...
        blend_trace: Dict = {
            "formula_notes": (
                "rule_score = SKILLS_WEIGHT*skills_score + WORK_STYLE_WEIGHT*work_style_score + "
                "SOFT_SKILLS_WEIGHT*soft_skills_score; "
                "blended_pre_spread = ml_w_effective * ml_score_0_100 + rule_w_effective * rule_tilt; "
                "ml/rule weights are deterministic constants; "
                ...
                f"hard cap to <= {self.DOMAIN_MISMATCH_FINAL_CAP:.0f} when skills_score < {self.DOMAIN_MISMATCH_SKILL_THRESHOLD:.0f}."
            ),
            "constants": {
                "ML_BLEND_WEIGHT": self.ML_BLEND_WEIGHT,
                "RULE_BLEND_WEIGHT": self.RULE_BLEND_WEIGHT,
                ...
                "WORK_STYLE_WEIGHT": self.WORK_STYLE_WEIGHT,
                "SOFT_SKILLS_WEIGHT": self.SOFT_SKILLS_WEIGHT,
                "DOMAIN_MISMATCH_SKILL_THRESHOLD": self.DOMAIN_MISMATCH_SKILL_THRESHOLD,
                "DOMAIN_MISMATCH_FINAL_CAP": self.DOMAIN_MISMATCH_FINAL_CAP,
        ...
        return {
            ...
            'ml_layer': ml_layer,
            'soft_skills_debug': soft_skills_debug,
            'occupation': {
    ...
    def _build_soft_skills_debug(
        self,
        *,
        use_gemini_fit: bool,
        parsed_soft_model: Optional[SoftSkillRequirements],
        user_soft_inferred: Dict[str, str],
    ) -> Dict:
        """
        API / UI payload: posting soft-skill demands (when parsed), inferred profile map,
        and a 0–100 display score when at least one posting dimension was scored.
        """
        user_inf = {k: user_soft_inferred.get(k) for k in _SOFT_SKILL_KEYS}

        demands_flat: Dict[str, str] = {}
        if use_gemini_fit and parsed_soft_model is not None:
            for k in _SOFT_SKILL_KEYS:
                v = _normalize_soft_demand_level(getattr(parsed_soft_model, k, None))
                if v:
                    demands_flat[k] = v

        dimensions_scored = len(demands_flat)
        soft_skills_score: Optional[float] = None
        if dimensions_scored > 0:
            scores: List[float] = []
            for key in _SOFT_SKILL_KEYS:
                dlevel = demands_flat.get(key)
                if not dlevel:
                    continue
                dn = _soft_level_to_num(dlevel)
                if dn is None:
                    continue
                ulevel = user_inf.get(key, "medium")
            ...
            if scores:
                soft_skills_score = float(round(sum(scores) / len(scores), 1))

        tiered = sum(1 for k in _SOFT_SKILL_KEYS if user_inf.get(k) in ("medium", "high"))
        ui_hint: Optional[str] = None
        if soft_skills_score is None:
            ui_hint = (
                "Soft skills not clearly extracted from posting. "
                f"Inferred from your profile: {tiered}/7 dimensions at medium or higher signal strength "
                "(no role-specific soft-skill demands were parsed from this posting to score against)."
            )

        return {
            "soft_skills_score": soft_skills_score,
            "soft_skills_extracted": demands_flat if demands_flat else None,
            "soft_skills_user_inferred": user_inf,
            "posting_had_soft_skill_requirements": bool(dimensions_scored > 0),
            "dimensions_scored": dimensions_scored,
            "soft_skills_ui_hint": ui_hint,
        }
    ...
    def _skills_alignment_pct(self, user_skills: List[str], job_skills: List[str]) -> float:
        """0–100 weighted domain/technical match based on overlap (same matcher as skill factor)."""
        ...
        user_coverage = len(covered_user) / n_user
        weighted_match_rate = 0.7 * job_coverage + 0.3 * user_coverage
        return max(0.0, min(100.0, weighted_match_rate * 100.0))
