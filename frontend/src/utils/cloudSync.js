...
const JOB_SCORE_SAVED_KEY = "neuroguide.jobScore.saved.v1";
...
export function buildJobWorkbenchBlob() {
  return {
    ...
    [JOB_SCORE_SAVED_KEY]: readJSON(JOB_SCORE_SAVED_KEY),
  };
}
  ...
  if (Object.prototype.hasOwnProperty.call(snap, "job_workbench")) {
    if (snap.job_workbench && typeof snap.job_workbench === "object") {
      ...
      try {
        ...
        window.localStorage.removeItem(JOB_SCORE_SAVED_KEY);
...
function applyJobWorkbenchBlob(blob) {
  ...
  if (blob[JOB_SCORE_SAVED_KEY] !== undefined) writeJSON(JOB_SCORE_SAVED_KEY, blob[JOB_SCORE_SAVED_KEY]);
